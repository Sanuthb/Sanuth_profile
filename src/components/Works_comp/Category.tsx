...
const Category = () => {
  ...
  const filteredItems = items.filter(item => item.category === selectedCategory);
    ...
    console.log(items[0].description.slice(0,10).concat("..."));
      ...
      <div className="mt-4 flex flex-wrap gap-6">
        {filteredItems.map((item, index) => (
          <div key={index} className="border p-4 mb-4 bg-white rounded-md w-[20rem] flex flex-col gap-2">
            <Image src={item.image.src} alt={item.title}  width={350} height={100} />
            <h2 className="text-lg font-bold">{item.title}</h2>
            <p>{item.description.length > 100 ? item.description.slice(0,80).concat("...") : item.description}</p>
            <Link href={`works/${item.id}`} className='bg-[#ffc739] p-2 rounded-lg'>View Product</Link>
          </div>
        ))}
      </div>
    </div>
